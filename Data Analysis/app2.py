...
def generate_sample_data(data, file_name, num_rows=200):
...
def load_data():
    ...
    # Attempt to convert 'FirstSeen' to datetime and handle errors
    data['FirstSeen'] = pd.to_datetime(data['FirstSeen'], errors='coerce')
    
    if data['FirstSeen'].isnull().any():
        st.error("Some 'FirstSeen' dates could not be converted. Check data format.")
...
def display_map_global(data, zoom_start=2):
    if data.empty:
...
if page == "Home":
    ...
    if 'FirstSeen' in data.columns:
        ...
        # Display map for the selected year
        display_map_global(year_data, zoom_start=2)

        # Display data table for the selected year
        if not year_data.empty:
            st.write(f"Starbucks locations opened in {selected_year}:")
            st.dataframe(year_data[['Name', 'Street1', 'City', 'CountryCode']].rename(columns={'Name': 'Store Name', 'Street1': 'Address', 'City': 'City', 'CountryCode': 'Country'}))
        else:
