...
#         st.error("FirstSeen column is missing from the dataset or does not contain any valid dates.")
    ...
    if 'FirstSeen' in data.columns:
        ...
        years = data['Year'].dropna().unique()
        years.sort()
        selected_year = st.slider('Select a Year', int(min(years)), int(max(years)), int(min(years)))
        ...
        if not year_data.empty:
            # Generate the map
            latitude_center = year_data['Latitude'].mean() if not year_data['Latitude'].isnull().all() else 0
            longitude_center = year_data['Longitude'].mean() if not year_data['Longitude'].isnull().all() else 0
            m = folium.Map(location=[latitude_center, longitude_center], zoom_start=3)
